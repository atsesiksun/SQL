...
if @temp_rows = @null_rows
	...
	begin
		...
		if @temp_rows > 1
			begin
			--store all table names that do not exist in database in variable @table_not_found
	...
	-- if number of rows in parameter is less than 2, will ask to provide more tables for comparison
	if @temp_rows < 2
		begin
			if @null_rows = 0
				begin
					...
					if  @null_rows > 0
						begin
							...
							if @temptable3_rows = 0 
								...
								-- and there are similar values, return:
								begin
									set @sql1 = 'Tables not found in database: ' + char(10) + cast(@table_not_found as nvarchar(max)) + char(10)
										+ 'Similar columns in ' + cast(@table_count as nvarchar(max)) + ' out of ' + cast(@temp_rows as nvarchar(max)) + 
